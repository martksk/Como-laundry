...
//   useEffect(() => {
//     setInView(true); // Trigger the animation once the component is mounted
//   }, []);
...
//         <div className={`text-6xl md:text-9xl tracking-widest font-bold text-blue-900 pl-20 ${
//             inView ? 'animate-fade' : ''
//           }`}>FAQ</div>
//       </div>
//       {faqData.map((faq, index) => (
...
//               <span className="text-xl md:text-2xl font-semibold text-blue-800">{faq.question}</span>
//               <span className={`transform transition-transform duration-500 ${visibleAnswers[index] ? 'rotate-180' : ''}`}>
//                 ▼
//               </span>
//             </div>
//           </button>
...
//               visibleAnswers[index] ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
//             }`}
//           >
...
export default function FAQ() {
        ...
        const observer = new IntersectionObserver(
            ([entry]) => {
                if (entry.isIntersecting) {
                    setInView(true); // Trigger animation when FAQ enters the viewport
                } else {
                    setInView(false); // Reset inView when FAQ exits the viewport
                }
            },
    ...
    const faqData = [
        ...
        {
            question: "ระยะเวลาการใช้งานของเครื่อง",
            answer: "อายุการใช้งานของเครื่องมีระยะเวลา 25 - 30 ปี"
        }
    ];

    return (
        <div ref={faqRef} className="flex flex-col text-black max-w-4xl mx-auto px-4">
            <div className="flex flex-col justify-center items-center text-black mb-16">
                <div className={`text-6xl md:text-9xl tracking-widest font-bold text-blue-900 pl-20 transition-opacity duration-1000 ease-in-out ${
